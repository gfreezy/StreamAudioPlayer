...
public final class URLAudioPlayer: NSObject, URLSessionTaskDelegate, URLSessionDataDelegate, @unchecked Sendable {
    private let lock = NSLock()
    private var _urlSessionTask: URLSessionDataTask? = nil
    private var urlSessionTask: URLSessionDataTask? {
        get { lock.withLock { _urlSessionTask } }
        set { lock.withLock { _urlSessionTask = newValue } }
    }
    private let url: URL?
    ...
    public func load(_ url: URL? = nil) {
        ...
        task = URLSession.shared.dataTask(with: u)
